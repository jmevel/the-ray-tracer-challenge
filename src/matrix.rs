...
impl Matrix<row_count, col_count> {
    ...
    pub fn view_transform<const ROW_COUNT: usize, const COL_COUNT: usize>(
        from: &Point,
        to: &Point,
        up: &Vector,
    ) -> Matrix<ROW_COUNT, COL_COUNT> {
        let forward = (to - from).normalize();
        let right = up.normalize().cross_product(&forward); // for -Z convention: let left = forward.cross_product(&up.normalize());
        let true_up = forward.cross_product(&right); // for -Z convention: let true_up = right.cross_product(&forward);

        let mut orientation = Matrix::identity_matrix();

        orientation.data[0][0] = right.x();
        orientation.data[0][1] = right.y();
        orientation.data[0][2] = right.z();
